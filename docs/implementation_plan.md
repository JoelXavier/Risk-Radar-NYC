...
1.  Write script `scripts/fetch_data.ts` to pull sample data (e.g., Manhattan subset first to save time).
2.  Implement `calculateScore.ts` logic.
3.  Generate `public/data/buildings_subset.geojson`.

### Phase 3: Map Integration
1.  Install `maplibre-gl`.
2.  Render the Map using a free vector tile source (e.g., Carto or Protomaps) or maintain just the GeoJSON overlay on a blank/minimal style.
3.  Load the GeoJSON source.
4.  Style buildings based on `properties.risk_score`.

### Phase 4: UI & Interactivity
1.  Build the `Sidebar` for viewing details.
2.  Implement click interactions (zoom to building, show details).
